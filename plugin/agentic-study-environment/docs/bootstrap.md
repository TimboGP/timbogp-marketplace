...
- **Typical flow:**
  ...
  5. It registers the project in `.studyenv/PROGRESS.md` (creating that tracker on first bootstrap), then prompts you to drop source materials into `source-materials/`. With `Tracking: local-only` this registration is skipped — the sub-project keeps only its own `PROGRESS.md`.
...
**Writes/updates:**
- `.studyenv/PROGRESS.md` — adds the Projects row and a dated Journal entry (creates the file on first use; otherwise appends without touching prior rows). **Skipped under `Tracking: local-only`.**
...
- **Tracking scope.** `Tracking: global` (default) registers the sub-project in the cross-project tracker — the umbrella mode. `Tracking: local-only` keeps just the sub-project's own `PROGRESS.md`, for dropping the harness onto a single existing project with no global index.
