...
- role-play a clinical case (`simulation`) or rehearse a research defense (`defense`)
...
- **Typical flow:**
  ...
  2. Decide a session type: `theory`, `practice`, or an overlay-specific `simulation`/`defense`.
...
- **Session types:** `theory` (concepts, definitions, intuitions, proofs on request), `practice` (work a scaffolded exercise grounded in the current topic), and overlay-only `simulation` (`Domain: speech-therapy`) and `defense` (`Domain: academic-research`). A theory session may include grounding practice and vice versa, but each session has one primary type.
